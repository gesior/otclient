var mapConfig = {
    ...
    // NPC & Monsters
    creaturesDataURL: 'creaturesData.php', // set 'false' to disable, set 'creaturesData.php' to make it load data
...
if (tmpLocation.length == 6 && tmpLocation[0] == 'zoom' && parseInt(tmpLocation[1]) && parseInt(tmpLocation[3]) && parseInt(tmpLocation[4]) && parseInt(tmpLocation[5])) {
	// #zoom,3,position,32000,32000,7
...
} else if (tmpLocation.length == 2 && tmpLocation[0] == 'npc') {
	// #npc,Xodet
    npcToLoad = decodeURI(tmpLocation[1]).replace('+', ' ').replace('+', ' ').replace('+', ' ');
} else if (tmpLocation.length == 2 && tmpLocation[0] == 'monster') {
	// #monster,Demon
    monsterToLoad = decodeURI(tmpLocation[1]).replace('+', ' ').replace('+', ' ').replace('+', ' ');
}
...
var CreaturesMap = {
        ...
        if (mapConfig.creaturesDataURL) {
            $.getJSON(mapConfig.creaturesDataURL, function (data) {
                if (data) {
                    ...
                    if (monsterToLoad != '') {
                        $('#monsterSearch').val(monsterToLoad);
                    }
                    $('#monsterSearch').chosen();
                    if (monsterToLoad != '') {
                        CreaturesMap.monsterSearch();
                    }
                    ...
                    if (npcToLoad != '') {
                        $('#npcSearch').val(npcToLoad);
                    }
                    $('#npcSearch').chosen();
                    if (npcToLoad != '') {
                        CreaturesMap.npcSearch();
                    }
                }
            });
        }
    ...
    redrawTowns() {
        for (var townMarkerId in CreaturesMap.visibleTowns) {
            var townMarker = CreaturesMap.visibleTowns[townMarkerId];
            CreaturesMap.map.removeLayer(townMarker);
        }
        CreaturesMap.visibleTowns = [];

        if (map.getZoom() < mapConfig.drawMonstersMinimumZoom) {
            for (var townId in CreaturesMap.towns) {
                var town = CreaturesMap.towns[townId];
                var divider = 22 - map.getZoom();
                var fontSize = 200 / divider;
                var townIcon = new L.divIcon({
                    iconSize: new L.Point(300, 60),
                    html: '<span style="color:white;font-size:' + fontSize + 'px">' + town.name + '</span>'
                });

                var townMarker = new L.marker(new L.LatLng(-((town.y) / 2048), ((town.x) / 2048)), {
                    icon: townIcon
                });
                townMarker.addTo(CreaturesMap.map);
                CreaturesMap.visibleTowns.push(townMarker);
            }
        }
    },
                ...
                if (monster.z == Map.currentFloor) {
                    ...
                    if (monster.look[1] > 0) {
                        var monsterIcon = new L.Icon({
                            iconSize: [2048 / divider, 2048 / divider], iconAnchor: [3072 / divider, 3072 / divider],
                            iconUrl: mapConfig.itemGeneratorURL + monster.look[1] + '.png'
                        });
                        positionChange.x += 1;
                        positionChange.y += 1;
                    } else {
                        var monsterIcon = new L.Icon({
                            iconSize: [4096 / divider, 4096 / divider], iconAnchor: [3072 / divider, 3072 / divider],
                ...
                if (npc.z == Map.currentFloor) {
                    ...
                    if (npc.look[1] > 0) {
                        var npcIcon = new L.Icon({
                            iconSize: [2048 / divider, 2048 / divider], iconAnchor: [3072 / divider, 3072 / divider],
                            iconUrl: mapConfig.itemGeneratorURL + npc.look[1] + '.png'
                        });
                        positionChange.x += 1;
                        positionChange.y += 1;
                    } else {
                ...
                if (player.currentPosition.z == Map.currentFloor) {
                    ...
                    if (mapConfig.streamOutfit && map.getZoom() > mapConfig.streamHideOutfitOnZoom) {
                        ...
                        if (player.outfit.typeEx > 0) {
                            playerIcon = new L.Icon({
                                iconSize: [2048 / divider, 2048 / divider],
                                iconAnchor: [3072 / divider, 3072 / divider],
                                iconUrl: mapConfig.itemGeneratorURL + player.outfit.typeEx + '.png'
                            });
                        } else {
                            playerIcon = new L.Icon({
                                iconSize: [4096 / divider, 4096 / divider],
                                iconAnchor: [3072 / divider, 3072 / divider],
                                iconUrl: url + 'id=' + player.outfit.type + '&addons=' + player.outfit.addons + '&head=' + player.outfit.head + '&body=' + player.outfit.body + '&legs=' + player.outfit.legs + '&feet=' + player.outfit.feet + '&mount=' + player.outfit.mount + '&direction=' + direction
                            });
                        }
                    } else {
...
var Map = {
    ...
    setCenter: function (position, teleport) {
        ...
        if (teleport) {
            map.panTo(new L.LatLng((positionY / 2048), (positionX / 2048)), {
                animate: false,
                duration: 0.0,
                easeLinearity: 1.0
            });
...
CreaturesMap.towns.push({name: "Thais", x: 32366, y: 32220});
CreaturesMap.towns.push({name: "Carlin", x: 32339, y: 31792});
