...
$spawnsFileContents = strtolower(file_get_contents($dataPath . $spawnsFile));
// LOAD MONSTERS
...
foreach ($monsters as $monster) {
    ...
    $monsterMapName = strtolower($monster['name']);
    if (strpos($spawnsFileContents, '"' . $monsterMapName . '"') === false) {
        file_put_contents('error.log', 'NOTICE: Monster does not exist on map: ' . $dataPath . $monstersDir . $monster['file'] . PHP_EOL, FILE_APPEND);
        continue;
    }
    ...
    $data = [
        'name' => ucfirst($monster['name']),
        'look' => $look
    ];
    $monstersData[$monsterMapName] = $data;
}
ksort($monstersData);
...
foreach (glob($dataPath . $npcDir . '*.xml') as $filePath) {
    ...
    $data = [
        'name' => $npcFileName, //$npcData['@attributes']['name'],
        'look' => $look
    ];
